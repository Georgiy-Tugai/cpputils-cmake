...
(defun cppcm-create-one-makefile (root-src-dir build-dir src-dir tgt mk)
  "try to create Makefile for flymake, require the project be compiled successfully at least once"
  (let (flag-make
        base-dir
        cm
        c-flags
  ...
  (let ((base src-dir)
    ...
    (when (file-exists-p cm)
      ...
      (dolist (tgt possible-targets)
        ...
        (if cppcm-write-flymake-makefile
          (cppcm-create-one-makefile root-src-dir build-dir src-dir tgt mk))
        )
      )
