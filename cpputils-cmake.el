  ...
  (let (tks
    ...
    (dolist (tk tks v)
      ...
      (if next-tk-is-included-dir ;; only for "-isystem"
        ...
        (cond
         ((string= (substring tk 0 3) "-I\"")
          (setq v (concat v " -I" (substring tk 3 (- (length tk) 1)) )))
...
Require the project be compiled successfully at least once."
  (let (flag-make
    ...
    (message "exe-full-path=%s" exe-full-path)

    (when exe-full-path
      (puthash (cppcm--exe-hashkey base-dir) exe-full-path cppcm-hash)
      (message "flag-make=%s" flag-make)
